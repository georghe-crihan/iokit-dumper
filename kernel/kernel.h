...
typedef struct MetaClass_Object {
    vm_offset_t VTable_ptr;
    vm_offset_t __unknown;
    vm_offset_t Parent_gMetaClass_ptr;
    vm_offset_t OSSymbol_ptr;
    uint32_t class_size;
} MetaClass_Object_t;

typedef struct OSSymbol_Object {
    vm_offset_t VTable_ptr;
    vm_offset_t __padding;
    vm_offset_t name_length;
    vm_offset_t name_ptr;
} OSSymbol_Object_t;
...
__attribute__((always_inline)) pointer_t read_kernel_memory(vm_map_t task, vm_offset_t addr, mach_vm_size_t size);
